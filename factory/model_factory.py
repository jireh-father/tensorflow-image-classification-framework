            ...
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % ("tower", i)) as scope:
                    ...
                    if config.model_name in model_factory.networks_map.keys():
                        ...
                        #                         name="inputs")
                        # tf.summary.image('input', inputs, config.num_input_summary)
                        logits, end_points = model_f(dataset.images)
                        ...
                        # inputs = tf.placeholder(tf.float32, shape=[None, config.input_size, config.input_size, config.num_channel],
                        #                         name="inputs")
                        tf.summary.image('input', dataset.images, config.num_input_summary)
                        model_result = build_model_f(dataset.images, config, is_training=is_training)
                        if isinstance(model_result, tuple):
                            logits = model_result[0]
                            end_points = model_result[1]
                        else:
                    ...
                    ops = None
                    if loss_f:
                        loss = loss_f(logits, dataset.labels, global_step, config, scope, opt)
