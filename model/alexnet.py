...
def build_model(inputs, config, is_training):
    end_points = {}
    net = tf.layers.conv2d(inputs, 96, 11, strides=4, padding="VALID", name="conv1", activation=tf.nn.relu)
    ...
    net = tf.layers.conv2d(net, 256, 5, padding="VALID", name="conv2", activation=tf.nn.relu)
