...
def build_model(inputs, config, is_training):
    end_points = {}
    net = tf.layers.conv2d(inputs, 96, 7, strides=2, padding="VALID", name="conv1",
    ...
    net = tf.layers.conv2d(net, 256, 5, strides=2, padding="VALID", name="conv2",
                           kernel_initializer=tf.constant_initializer(0.01),
                           bias_initializer=tf.zeros_initializer(), activation=tf.nn.relu)
    ...
    net = tf.reshape(net, [-1, int(net.get_shape()[1]) * int(net.get_shape()[2]) * int(net.get_shape()[3])],
                     name="reshape")
    net = tf.layers.dense(net, 4096, kernel_initializer=tf.constant_initializer(0.01),
                          bias_initializer=tf.zeros_initializer(), activation=tf.nn.relu, name="fc6")
    net = tf.layers.dropout(net, training=is_training)
    net = tf.layers.dense(net, 4096, kernel_initializer=tf.constant_initializer(0.01),
                          bias_initializer=tf.zeros_initializer(), activation=tf.nn.relu, name="fc7")
    net = tf.layers.dropout(net, training=is_training)
    net = tf.layers.dense(net, config.num_class, kernel_initializer=tf.constant_initializer(0.01),
                          bias_initializer=tf.zeros_initializer(), activation=tf.nn.relu, name="fc8")
